...
import commomBeans.CondPagamento;
...
public class DAOCondPagamento {
	...
	public void alterar(CondPagamento condPag) throws SQLException {
		String sql = "update cond_pagamento set nome_cond_pag=?, indice_juros=?,"
	...
	public void cadastrar(CondPagamento condPag) throws SQLException {
		String sql = "insert into cond_pagamento (codi_cond_pag,  nome_cond_pagamento , indice_juros,"
				+ " desconto_acrescimo, gera_conta_receber,  quant_parcelas, tipo_numeracao, "
	...
	public ArrayList<CondPagamento> pesquisaString(String str) {
		String sql = "select * from cond_pagamento where codi_cond_pag ~* ?  or nome_cond_pag ~* ? order by codi_cond_pag;";
	...
	public String pesquisaNomeCodigo(String codigo) {
		String sql = "select nome_cond_pag from cond_pagamento where codi_cond_pag =?;";
		String nome = null;
		c.conectar();
		try {
			prepStm = c.getCon().prepareStatement(sql);
	...
	public String pesquisaCodigoNome(String nome) {
		String sql = "select codi_cond_pag from cond_pagamento where nome_cond_pag =?;";
		String codigo = null;
		c.conectar();
		try {
			prepStm = c.getCon().prepareStatement(sql);
	...
	public CondPagamento pesquisaCondPagNome(String nome) {
		String sql = "select * from cond_pagamento where nome_cond_pag =?;";
		String codigo = null;
		c.conectar();
		try {
			prepStm = c.getCon().prepareStatement(sql);
	...
	public CondPagamento pesquisaCondPagCodigo(String codigo) {
		String sql = "select * from cond_pagamento where codi_cond_pag =?;";
		c.conectar();
		try {
			prepStm = c.getCon().prepareStatement(sql);
			prepStm.setString(1, codigo);
	...
	public int consultaUltimo() {
		c2.conectStm();
		result = c2.query("SELECT max(seq_cond) FROM cond_pagamento;");
		c2.disconect();
		try {
			result.next();
	...
	public void reservaCodigo(String codigo) throws SQLException {
		String sql = "insert into cond_pagamento (codi_cond_pag) values (?)";
		c.conectar();
		prepStm = c.getCon().prepareStatement(sql);
		prepStm.setString(1, codigo);
		prepStm.executeUpdate();
	...
	public boolean remover(CondPagamento condPag) {
		String sql = "delete from cond_pagamento where codi_cond_pag=?;";
		c.conectar();
		try {
			prepStm = c.getCon().prepareStatement(sql);
			prepStm.setString(1, condPag.getCodiCondPag());
