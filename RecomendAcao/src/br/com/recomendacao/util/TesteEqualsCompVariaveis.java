 ...
 *
 * @version Classe criada para sanar a dúvida sobre comparação de objetos usando
 *          o método .equals e o operador ==. O presente estudo levou a
 *          sobrescrever o metodo.equals da classe CotaçaoYahoo equals -->
 *          Comparações dentro do Heap da JVM as características do objeto ==
 *          --> Comparações fora do Heap da JVM se a referência do objeto aponta
 *          para o mesmo obeto comparado obs.: esse estudo vale para objetos e
 *          não para tipos primitivos; O maior resultado desse estudo é que
 *          deve-se usar o gerador de código automático do eclipse para gerar os
 *          métodos equals e hashcode sendo dessa forma mais eficiente e segura
 *          a comparação.
 */
public class TesteEqualsCompVariaveis {
	...
	public void testaArrayList() {
		...
		System.out.println("Tamanho da lista de últimos: "
	...
	/**
	 * Para implementar o hashCode() é necessário prestar atenção nas regras de
	 * sobrescrição para que se escreva uma sintaxe válida. De acordo com a
	 * documentação do Java, esta é a sobrescrição correta do método: que deve
	 * estar na clase a qual se quer utilizar os algorítmos de comparaação:
	 * 
	...
	public void testaHashMap() {
		...
		/*
		 * Criaremos uma terceira cotação e iremos efetuar uma busca para ver se
		 * o algoritimo está, de fato, funcionando. Não será encontrado o objeto
		 * porque Primeiro: não o inserimos no HashMap Segundo: o tamanho do
		 * nome não bate com nenhum tamanho de nome inserido.
		 */
		 ...
		 * - não bate com o nome da cotação que está lá. Por isto é importante a
		 ...
		 * recuperar o objeto. A ordem de parametros implica na mudança da busca
		 * nesse caso posso achar a cotação pela String de nome que foi
		 ...
		 */
		// cot5 = hash.get(" Consigo acesso à que estão no hash Cotação 1");//
	...
	public void testaHashSet() {
		...
		System.out.println("cot3 .equals cot1? " + cot3.equals(cot1));// True
		System.out.println();
		System.out.println("Contem cot3? hash.contains "
				+ hashSetCotacao.contains(cot3)); // False
		System.out.println();
		System.out.println("Contem cot1? hash.contains "
				+ hashSetCotacao.contains(cot1)); // False
		System.out.println();
