...
public class ControlaAtivo {
	...
	public JTable pesqAtivoTabelaUltimas() {
		...
		for (int i = 0; i < listUlt.size(); i++) {
			...
			if (variacao <= -0.3 | variacao >= 0.3) {
				Object linha[] = { listUlt.get(i).getIdYahoo(), listUlt.get(i).getPreMax(), listUlt.get(i).getPreMin(),
						listUlt.get(i).getPreFec(), listUlt.get(i).getVariacao(), hora + " " + diaMes };
	...
	public JTable pesqAtivoTabela(String str) {
		...
		for (int i = 0; i < listAtv.size(); i++) {
			Object linha[] = { listAtv.get(i).getIdNeg(), listAtv.get(i).getNomeRes(), listAtv.get(i).getCotacao() };
			modelotabela.addRow(linha);
		}
	...
	public JTable pesqNomeTabelaAdicionaAtivoOp(String nome) {
		...
		for (int i = 0; i < listAtv.size(); i++) {
			Object linha[] = { listAtv.get(i).getIdNeg(), listAtv.get(i).getNomeRes(), listAtv.get(i).getTpMerc() };
			modelotabela.addRow(linha);
		}
		tabela1.setShowGrid(true);
		tabela1.setModel(modelotabela);
