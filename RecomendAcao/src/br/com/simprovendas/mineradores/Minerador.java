package br.com.simprovendas.mineradores;
...
import br.com.simprovendas.dao.DAOOrdemBovespa;
import br.com.simprovendas.indicadoresAT.MediaMovelSimples;
...
public class Minerador implements Runnable {
	...
	public void recomendaVenda(String idNeg, int dias) throws InterruptedException {
		...
		if (atv.getListCot().get(0).getQuaToTit() > 200) {
			...
			areaAtu.append("Tamanho da lista de valores de Fechamento: " + atv.getListCot().size() + "\n");
			Thread.sleep(150);
			areaAtu.append("Tamanho da lista de valores da Média: " + listMM.size() + "\n");
			Thread.sleep(150);
			...
			for (int i = 0; i < k; i++) {
				if (listMM.get(i) < valores.get(i)) {
					...
					areaAtu.append(("Venda recomendada por MM de " + dias + " dias  na data: "
							+ atv.getListCot().get(i).getDataCotacao() + " ::: Valor da Média: " + listMM.get(i)
							+ "  | VENDA NO PRE FEC: " + valores.get(i) + "\n"));
					...
					fuse.setDataInicio(atv.getListCot().get(i).getDataCotacao());
					fuse.setQuant(300);
					...
					ope.setDataHoraExec(atv.getListCot().get(i).getDataCotacao());
					ope.setTipoOp("Venda");
					...
					while (listMM.get(j) < valores.get(j) & (j < k)) {
						...
						// + " dia(s)\n");
						areaAtu.append("Permaneceu vendido: " + contDias + " dia(s)\n");
						Thread.sleep(150);
					...
					areaAtu.append("Saiu da operação vendida...Foram " + contDias + " de operação\n");
					areaAtu.append("Valor de RECOMPRA no preço de Fec.: " + valores.get(j) + " ::: Valor da média: "
							+ listMM.get(j) + " ::: Na data =>> " + atv.getListCot().get(j).getDataCotacao() + "\n");
					Thread.sleep(150);
					areaAtu.setCaretPosition(areaAtu.getDocument().getLength());
					ope = new Operacao();
					ope.setCodiAtivo(atv.getIdNeg());
					ope.setCodiOrdem(fuse.getCodiFuse());
					ope.setDataHoraExec(atv.getListCot().get(j).getDataCotacao());
					...
					daoOpe.cadastrar(ope);
					fuse.setLucroPrejuizo(fuse.getPrecStartCompra() - fuse.getPrecStartVenda());
	...
	public void recomendaCompra(Ativo atv) throws InterruptedException {
		...
		System.out.println(atv.getNomeRes());
		System.out.println(atv.getListCot().size() + " valores de movimento para análise");
		int periodo = Integer.parseInt(JOptionPane.showInputDialog("Deseja a média de quantos dias?"));
		System.out.println("Comparando com MM de: " + periodo + " dias.");
		listMM = calculaMedia.calcula(periodo, valores);
		int k = listMM.size() - 1;
		System.out.println("Tamanho da lista de valores da Média: " + listMM.size());
		System.out.println("Tamanho da lista de valores de Fechamento: " + listCot.size());
		System.out.println("Serão utilizados " + k + " valores.");
		for (int i = 0; i < k; i++) {
			if (listMM.get(i) > valores.get(i)) {
				System.out
						.println("Compra recomendada por MM " + periodo + " na data: " + listCot.get(i).getDataCotacao()
								+ " Valor da Média: " + listMM.get(i) + "  | COMPRA NO PRE FEC: " + valores.get(i));
				int j = i;
				int contDias = 0;
				Thread.sleep(300);
				...
				while (listMM.get(j) > valores.get(j) & (j < k)) {
					contDias++;
					System.out.println("Permaneceu comprado: " + contDias + " dia(s)");
					i = j;
					j++;
					Thread.sleep(300);
				}
				System.out.println("Saiu da operação comprada...Foram " + contDias + " de operação!!");
				System.out.println("Valor da média: " + listMM.get(j) + " ::: Valor da venda no Preço de Fec.: "
						+ valores.get(j) + "::: Na data =>> " + listCot.get(j).getDataCotacao() + "\n");
			}
			Thread.sleep(300);
		}
	}
