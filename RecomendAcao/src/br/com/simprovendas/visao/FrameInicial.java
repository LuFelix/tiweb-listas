...
import br.com.simprovendas.controle.ControlaPosicaoFinanceira;
...
import br.com.simprovendas.util.AtualizaCotacaoAutHashSet;
import br.com.simprovendas.util.ManipulaArquivoTxt;
import br.com.simprovendas.util.SelecionaArquivo;
...
public class FrameInicial {
	...
	public FrameInicial() {
		...
		frmPrincipal.setTitle(
				"::- S.O.R.T.E. -:: - Sistema Original Resultante de Trabalhos Escolares - Salutar Alimentos Seguros");
		...
		// Text Fields
		lblPesquisa = new JLabel(new ImageIcon("//home//luciano//SIMPRO//SIMPRO//images//ic_search_24.png"));
		...
		btnSalvar.setBounds(430, 0, 90, 30);
		...
		btnImportar.setBounds(10, 30, 115, 23);
		btnLocalizar = new JButton("Localizar");
		btnLocalizar.setBounds(10, 60, 115, 23);
		...
		btnLocalizar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				manTxt = new ManipulaArquivoTxt();
				if (manTxt.verificaArquivo(arquivoAberto)) {
					String busca = JOptionPane.showInputDialog("Informe a sequência a ser encontrada no arquivo:");
					int quantidadeEncontrada = manTxt.buscarQuantString(busca, arquivoAberto);
					if (quantidadeEncontrada > 0) {
						JOptionPane.showMessageDialog(null, "A sequência ===>> " + busca + " <<=== foi encontrada "
								+ quantidadeEncontrada + " vezes no arquivo.");
					} else {
						JOptionPane.showMessageDialog(null, "A sequência==>  " + busca + "  <== não encontrada.");
					}
				}
			}
		});
		...
		txtfPesquisa.addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent tecla) {
				...
				if (tecla.getExtendedKeyCode() == 27) {// ESC
					...
					nome = txtfPesquisa.getText();
					setTabela(contCondPag.pesqNomeTabelaAdicionacondPagamentoAopedido(nome));
					scrLista.setViewportView(getTabela());
				}
			}
	...
	static int verificaPainelDetalhes() {
		...
		if (getPainelVisualiza().getClass().equals(PainelProdutos.class)) {
			...
			return 2;
		} else if (getPainelVisualiza().getClass().equals(PainelPedidos.class)) {
			return 3;
		} else if (getPainelVisualiza().getClass().equals(PainelCentroCusto.class)) {
			return 4;
		} else if (getPainelVisualiza().getClass().equals(PainelServico.class)) {
			return 5;
		} else if (getPainelVisualiza().getClass().equals(PainelCondPagamento.class)) {
			return 6;
		} else if (getPainelVisualiza().getClass().equals(PainelOrdServicos.class)) {
			return 7;
		} else if (getPainelVisualiza().getClass().equals(PainelTabelaPreco.class)) {
			return 8;
		} else if (getPainelVisualiza().getClass().equals(PainelConta.class)) {
			return 9;
		} else if (getPainelVisualiza().getClass().equals(PainelGrupoSubgrupo.class)) {
			return 10;
		} else if (getPainelVisualiza().getClass().equals(PainelFuse.class)) {
			return 11;
		} else if (getPainelVisualiza().getClass().equals(PainelOperacao.class)) {
			return 12;
		} else {
			return 0;
		}
